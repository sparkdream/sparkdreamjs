...
export class LCDQueryClient {
  ...
  prices = async (params: QueryPricesRequest): Promise<QueryPricesResponse> => {
    ...
    const endpoint = `akash/oracle/v1/prices`;
  ...
  params = async (_params: QueryParamsRequest = {}): Promise<QueryParamsResponse> => {
    const endpoint = `akash/oracle/v1/params`;
  ...
  aggregatedPrice = async (params: QueryAggregatedPriceRequest): Promise<QueryAggregatedPriceResponse> => {
    const endpoint = `akash/oracle/v1/aggregated_price/${params.denom}`;
    return await this.req.get<QueryAggregatedPriceResponse>(endpoint);
  };
}