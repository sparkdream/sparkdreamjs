...
export class QueryClientImpl implements Query {
  ...
  deployments = async (request: QueryDeploymentsRequest): Promise<QueryDeploymentsResponse> => {
    ...
    const promise = this.rpc.request("akash.deployment.v1beta5.Query", "Deployments", data);
  ...
  deployment = async (request: QueryDeploymentRequest): Promise<QueryDeploymentResponse> => {
    ...
    const promise = this.rpc.request("akash.deployment.v1beta5.Query", "Deployment", data);
  ...
  group = async (request: QueryGroupRequest): Promise<QueryGroupResponse> => {
    ...
    const promise = this.rpc.request("akash.deployment.v1beta5.Query", "Group", data);
    return promise.then(data => QueryGroupResponse.decode(new BinaryReader(data)));
  };
  /* Params returns the total set of deployment parameters. */
  params = async (request: QueryParamsRequest = {}): Promise<QueryParamsResponse> => {
    const data = QueryParamsRequest.encode(request).finish();
    const promise = this.rpc.request("akash.deployment.v1beta5.Query", "Params", data);
